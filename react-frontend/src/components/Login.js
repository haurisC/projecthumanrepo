...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (oauthError) {
      setFormErrors({ 
        general: decodeURIComponent(oauthMessage || 'OAuth authentication failed') 
      });
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setFormErrors({ general: 'Failed to initiate Google sign-in. Please try again.' });
    } finally {
  ...
  <div className="max-w-md mx-auto my-12 p-5 bg-white rounded shadow">
    <div className="text-center mb-8">
      ...
      {/* Google Sign-In Button, I used an AI to generate  the button so if it looks weird js let me know */}
      <div className="mt-4">
        <div className="relative">
        ...
        <button
          ...
          className="w-full mt-4 py-3 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
        >
          {oauthLoading ? (
            <>
